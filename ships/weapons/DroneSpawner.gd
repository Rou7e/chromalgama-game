...
export var open_hangar = true

export var launched_ship = preload("res://ships/tellurian_ships/tel_t1.tscn")
export var projectile = preload("res://ships/base_stuff/EnemyController.tscn")

func _ready():
	pass

func fire(excludes, subtick_time):
	if get_parent().get_node("ShipCargo2") is AnimatedSprite and open_hangar:
